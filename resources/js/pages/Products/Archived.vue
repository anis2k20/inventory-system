  ...
  <AppLayout>
    <div class="mx-auto p-6">
      <div class="mb-6 flex items-center justify-between">
        <h1 class="text-3xl font-bold">Archived Products</h1>
        <Link href="/products" class="inline-flex items-center rounded-md bg-blue-600 px-4 py-2 text-white hover:bg-blue-700">
          Back to Products
        </Link>
      </div>

      <div class="mb-6">
        <Input v-model="search" @input="debouncedSearch" placeholder="Search archived products..." class="max-w-sm" />
      </div>

      <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-6">
        <Card v-for="product in products.data" :key="product.id" class="transition-shadow hover:shadow-lg opacity-75"
          ><CardHeader>
          ...
          <CardContent class="space-y-1">
            ...
            <p class="text-xs text-gray-500">Deleted at: {{ new Date(product.deleted_at).toLocaleDateString() }}</p>
          </CardContent>
          <CardFooter class="flex gap-2">
            <Button @click="restoreProduct(product.id)" variant="default" size="sm">
              Restore
            </Button>
            <Button @click="openDialog(product.id)" variant="destructive" size="sm">
              Delete Permanently
            </Button>
          </CardFooter>
        </Card>
      ...
      <Dialog v-model:open="dialogOpen">
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" @click="confirmForceDelete">Delete Permanently</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
const debouncedSearch = (() => {
  ...
  return () => {
    ...
    timeout = setTimeout(() => {
      router.get('/products/archived', { search: search.value, page: 1 }, { preserveState: true });
    }, 300);
  };
})();
...
const goToPage = (page: number) => {
  ...
  router.get('/products/archived', { page, search: search.value }, { preserveState: true });
};
...
const confirmForceDelete = () => {
  if (productIdToDelete.value !== null) {
    router.delete(`/products/${productIdToDelete.value}/force-delete`, {
      onSuccess: () => {
        closeDialog();
      },
    });
